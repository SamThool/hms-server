...
const dischargePatient = async (req, res) => {
  try {
    ...
    // add discharge data
    patient.discharge = req.body;

    // remove occupiedBedId
    patient.occupiedBedId = undefined;
    // OR: delete patient.data.occupiedBedId;

    await patient.save();

    return res
      .status(200)
      .json({ message: "Patient discharged", data: patient });
  } catch (error) {
